...
function createMetadataPrompt(componentContext) {
...
let selectedModel = 'claude-sonnet-4-20250514'; // Default to Sonnet 4
...
${metadata.accessibility ? `Accessibility: ${metadata.accessibility}` : ''}`;
        ...
        try {
            ...
            if (metadata.slots && metadata.slots.length > 0) {
                ...
                function findAllInstances(parent) {
                    const instances = [];
...
async function applySlotPlaceholders(instance, slots) {
    try {
        ...
        slots.forEach((slotName) => {
            ...
            if (targetLayer) {
                ...
                if ('strokes' in targetLayer) {
                    ...
                    targetLayer.strokeWeight = 1;
                    targetLayer.strokeAlign = 'INSIDE';
                    targetLayer.dashPattern = [4, 4];
                }
            }
            else {
...
async function createComponentProperties(component, metadata) {
...
async function processEnhancedAnalysis(data, node) {
    ...
    if (data.audit && data.audit.consistencyIssues && data.audit.consistencyIssues.length > 0) {
        audit.consistency = data.audit.consistencyIssues.map((issue) => ({
            check: issue,
            status: 'warning'
        }));
    }
...
async function extractDesignTokensFromNode(node) {
    ...
    async function traverseNode(currentNode) {
        ...
        if ('fillStyleId' in currentNode && currentNode.fillStyleId) {
            ...
            stylePromises.push(figma.getStyleByIdAsync(styleId)
                .then((style) => {
                if (style && style.name) {
        ...
        if ('fills' in currentNode && currentNode.fills && Array.isArray(currentNode.fills) && !currentNode.fillStyleId) {
        ...
        else if (currentNode.fillStyleId) {
            console.log(`🎨 ✅ Skipping fill extraction - using fillStyleId:`, currentNode.fillStyleId);
        }
        // Extract colors from strokes ONLY if no strokeStyleId exists
        if ('strokes' in currentNode && currentNode.strokes && Array.isArray(currentNode.strokes) && !currentNode.strokeStyleId) {
        ...
        else if (currentNode.strokeStyleId) {
